...
Chat with your uploaded documents using a retrieval-augmented generation pipeline that is biased toward grounded, citation-backed answers.
...
- **Answer generation + verification** (`frontend/app.py`, `backend/app/verification.py`): The model writes a single paragraph from the selected evidence. A second pass judges whether the draft is supported by the excerpts; unsupported answers are replaced with a refusal, and citations are hidden when evidence is weak. This two-pass guard reduces hallucinations without over-refusing.
- **Routing + toggles** (`frontend/app.py`): Users can turn documents on/off; routing metadata is saved with each message. All retrieval, rerank, and verification decisions are stored in the DB so you can audit what evidence produced a reply.

## Data flow
1) **Upload** files in the Streamlit UI (`frontend/app.py`). Files are saved under `data/raw/` and ingested.
...
4) **Answer**: The model writes from the curated context; a verifier checks support before the response and evidence are shown.

## Quick start (local)
- Set `OPENAI_API_KEY` in `frontend/.streamlit/secrets.toml`.
- Install deps: `pip install -r requirements.txt`.
- Run the Streamlit UI: `streamlit run frontend/app.py`.
